...
if [ $# -ne 6 ]; then
	echo "Wrong number of arguments. Example: ./run_seed.sh opt 300 a1 1 local_search seed"
	exit -1
fi
...
taskset 0x00000003 $EXEC -t $TLIM -p $PROBLEM -i $INPUT -o $OUTPUT -h $HEURISTIC -name -s $SEED
