...
lines=`wc -l seeds.txt | awk '{print $1}'`
for i in `seq 1 $lines`; do
    seed=$(awk 'NR=='$i seeds.txt)
    taskset 0x00000003 $EXEC -t $TLIM -p $PROBLEM -i $INPUT -o $OUTPUT -h $HEURISTIC -name -s $seed > ${dir}/seed_${i}.txt
done
